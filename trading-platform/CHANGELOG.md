...
## Unreleased — Milestone 2 follow-up: Interactive Brokers adapter

- Vendor/broker decision resolved (`KNOWN_ISSUES.md` #1/#2): **Interactive Brokers**, for both market data and future paper/live execution.
- Added `InteractiveBrokersMarketDataProvider` (`modules/market_data/adapters/interactive_brokers_provider.py`) implementing `MarketDataProvider` via `ib_async`: maps our `timeframe` strings to IB bar sizes, uses `ContFuture` for continuous CME futures history, computes IB duration strings from the requested range, and translates `BarData` to our `Candle` domain model (including normalizing naive bar timestamps to UTC).
- Added `ib_connection.py` (connect/disconnect against an already-running TWS/IB Gateway) and wired a FastAPI lifespan hook in `app.main` that establishes one persistent IB connection at startup when `market_data_provider=interactive_brokers` — not per-request, since a TWS socket connection is stateful and rate-limited. Default stays `"replay"` so existing dev/test flows are unaffected.
- **Honest status: not verified against a live IB connection.** This sandbox has no running TWS/IB Gateway, no IB account, and no path to one — implementing and unit-testing the adapter (9 new tests against a fake IB client covering contract construction, bar-size mapping, duration-string edge cases, and timezone handling) is what could be done here. Closing Milestone 2's roadmap gate ("demonstrated backfill + live ingestion against real vendor data") requires the user to run this against their own TWS/IB Gateway with CME market data permissions for MES.
- Known gap, deliberately deferred: no chunking/pacing logic for IB's historical-data rate and per-bar-size duration limits — building that against rules that can't be verified from here risks guessing, so it's left for when real usage surfaces the actual constraints.
- 61 tests total, all passing; `ruff check` and `mypy --strict` both clean.
